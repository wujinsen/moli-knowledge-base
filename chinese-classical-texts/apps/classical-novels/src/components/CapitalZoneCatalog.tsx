...
export default function CapitalZoneCatalog({
  nodes,
  bookSlug,
  accent,
  accentSoft,
  ...
  const byZone = useMemo(() => {
    const map = new Map<CapitalZone, CapitalMapNode[]>();
    for (const z of CAPITAL_ZONE_ORDER) map.set(z, []);
    for (const n of nodes) {
      const list = map.get(n.zone) ?? [];
      list.push(n);
      map.set(n.zone, list);
    }
    return CAPITAL_ZONE_ORDER.map((z) => ({ zone: z, items: map.get(z) ?? [] })).filter(
      (g) => g.items.length > 0,
    );
  }, [nodes]);

  const setAll = (open: boolean) => {
    rootRef.current?.querySelectorAll('details.capital-catalog-group').forEach((d) => {
  ...
  return (
    ...
    >
      <div className="flex shrink-0 items-center justify-between gap-2 border-b border-white/10 px-3 py-2.5">
        <span className="text-xs font-medium text-slate-300">都外名录</span>
      ...
      <div className="min-h-0 flex-1 overflow-y-auto px-2 py-2">
        {byZone.map(({ zone, items }) => (
          <details key={zone} className="capital-catalog-group garden-catalog-group mb-2 last:mb-0" open>
            <summary className="garden-catalog-summary cursor-pointer list-none rounded-md px-2 py-1.5 text-sm font-semibold text-slate-200 hover:bg-white/5">
              <span style={{ color: accentSoft }}>{CAPITAL_ZONE_LABELS[zone]}</span>
