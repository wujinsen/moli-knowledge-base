...
export default function ItemsCrossIndex({ index, bookSlug, bookName }: Props) {
  ...
  return (
    <div className="items-cross-index">
      <div className="mb-4 flex flex-wrap items-center gap-3">
        ...
        <input
          ...
          placeholder={mode === 'chapter' ? '筛选章回 / 名物…' : '筛选人物 / 名物…'}
          ...
          filtered.map((g) => (
            ...
            >
              <div className="mb-3 flex flex-wrap items-baseline gap-2">
                ...
                <span className="text-xs text-slate-500">{g.rows.length} 条名物</span>
              </div>
              <ul className="space-y-2">
                {g.rows.map((row) => (
                  <li key={row.id} className="flex flex-wrap items-start gap-2 text-sm">
                    <a
                      href={`/${bookSlug}/i/${row.id}`}
