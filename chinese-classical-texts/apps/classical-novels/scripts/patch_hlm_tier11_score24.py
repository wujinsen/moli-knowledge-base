#!/usr/bin/env python3
"""/dream 第十一梯队 — score≤24 全量压至 ≥25。

- score=23 / score=24：自动补 1 条 guard 可核 relations（+2）
- 补关系失败且 score=24 且 in<5：hub 互链（+1）
- 仍失败：全书回扫 co-occur 再试

用法: python scripts/patch_hlm_tier11_score24.py [--dry-run]
"""
...
def patch_one(
    ...
    if score == 24 and info["inbound"] < 5:
...
def main() -> None:
    ...
    if not args.dry_run:
        thin = sum(1 for info in pages.values() if page_score(info) <= 24)
        print(f"remaining score<=24: {thin}")
        if stuck:
