#!/usr/bin/env python3
"""红楼梦人物 frontmatter「关键物品」→ 图鉴「信物」：剔除饮食/宴饮/诊脉等误同步项。

用法:
  python scripts/prune_hlm_keepsakes.py [--dry-run]
...
from _item_wiki import filter_hlm_keepsake_ids, list_item_catalog
...
def prune_characters(catalog: dict[str, dict], *, dry_run: bool) -> int:
    ...
    for path in sorted(char_dir.glob("*.md")):
        ...
        new = filter_hlm_keepsake_ids(old, catalog)
        if old == new:
            continue
        updated += 1
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Prune 红楼梦 关键物品 to keepsakes only")
    ...
    print(
        ...
        f"人物 {n_chars} · hlm_bestiary_fields 关键物品行 {n_fields}"
    )
    if n_chars or n_fields:
        print("提示: python scripts/build_hlm_bestiary_json.py && python scripts/seed_hlm_bestiary.py", file=sys.stderr)
