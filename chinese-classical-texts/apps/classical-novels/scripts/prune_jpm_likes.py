#!/usr/bin/env python3
"""金瓶梅人物图鉴「喜好」：剔除帮闲差事/店铺/组织关系等误填项。

用法:
  python scripts/prune_jpm_likes.py [--dry-run]
  python scripts/build_jpm_bestiary_json.py && python scripts/seed_jpm_bestiary.py
"""
...
from _item_wiki import filter_jpm_like_ids, list_item_catalog
...
def patch_extra_source(catalog: dict[str, dict], cids: set[str], *, dry_run: bool) -> int:
    ...
    for m in list(re.finditer(r'"([^"]+)":\s*\{[^{}]*?"喜好":\s*\[([^\]]*)\]', text)):
        ...
        filtered = filter_jpm_like_ids(ids, catalog, cids, relations=rel)
        if filtered == ids:
            continue
        changed += 1
...
def prune_characters(catalog: dict[str, dict], cids: set[str], *, dry_run: bool) -> int:
    ...
    for path in sorted(char_dir.glob("*.md")):
        ...
        rel = fm.get("relations") or []
        new = filter_jpm_like_ids(old, catalog, cids, relations=rel)
        if old == new:
            continue
        updated += 1
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Prune 金瓶梅 喜好 to valid semantics")
    ...
    n_extra = patch_extra_source(catalog, cids, dry_run=args.dry_run)
    print(
        f"[{BOOK}] {'(dry-run) ' if args.dry_run else ''}"
        f"人物 {n_chars} · jpm_bestiary_fields {n_extra}"
    )
    if n_chars or n_extra:
        print(
            "提示: python scripts/build_jpm_bestiary_json.py && python scripts/seed_jpm_bestiary.py",
            file=sys.stderr,
        )
