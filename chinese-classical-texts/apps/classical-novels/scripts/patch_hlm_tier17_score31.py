#!/usr/bin/env python3
"""/dream 第十七梯队 — score≤31 压至 ≥32。

- 循环：trust_guard 可核 relations（+2）；plot≤3 补情节；hub 互链
- 全书 183 页（score 30–31）→ 目标全库最低 32

用法: python scripts/patch_hlm_tier17_score31.py [--dry-run]
"""
...
from patch_hlm_tier10_score22 import add_hub_link, add_relations, pick_hub_source  # noqa: E402
...
def patch_one(
    ...
    for attempt in range(5):
        ...
        rel = pick_relation(
            cid, pages, all_fms, chapter_cache, full_scan=(attempt >= 1)
        )
        if rel:
            fm = dict(info["fm"])
...
def main() -> None:
    ...
    remaining = sum(1 for info in pages.values() if page_score(info) < TARGET)
    print(f"remaining score<{TARGET}: {remaining}")
    if stuck:
