...
lines = [f"score=26: {len(rows)} pages\n"]
for flag in ("plot", "rel", "inbound"):
...
lines.append("--- detail (plot<=3, rel<=4, in<5) ---")
for cid, d in sorted(rows, key=lambda x: (x[1]["plot"], x[1]["rel"], -x[1]["inbound"])):
    if d["plot"] <= 3 and d["rel"] <= 4 and d["inbound"] < 5:
        lines.append(
            f"{cid}\tplot={d['plot']}\trel={d['rel']}\tin={d['inbound']}\tw={d['weight']}"
        )

out = Path(__file__).resolve().parent / "_score26_scan.txt"
